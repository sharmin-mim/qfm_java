...
##	  4. COMMAND: perl data_generator.pl  -ntaxa n 
##	  	 where,
...
my $max_cut_dir = "/home/mimo/research/script/qmc";
my $orig_max_cut_dir = "/home/mimo/research/script/qmc/max-cut-tree";
my $simulation_src_dir = "/home/mimo/research/script/qmc";
my $local_simulation_src_dir = ".";
# my $start_step = "MXC";
my $start_step = "FILTER";
#goto $start_step;
my $paup_dir="/home/mimo/research/packages/paup4a166_ubuntu64";
...
#my @qf = (1.5, 2.0);   
my @qf = (1.5, 2.0, 2.8); # quartet factor. Here number of quartet = (number of taxa)^(qf)  
my $model_tree;
...
my $replicateNo;
 ...
 my $triplets_src_dir =  "/home/mimo/research/script/qmc";
...
my $line = "#NEXUS\n".
    "begin rates;\n".
    "simulate diversemodel=bdback seed=$seed ntaxa=$ntaxa T=0;\n".
...
`./r8s -b -f r8s.in > r8s-tree.out`;


open (IN,"r8s-tree.out"); 
while (<IN>) { 
    print $_;
    if ($_ =~ /tree\sSIMTREE\s=/) { 
	open (TREE, ">random-tree.dat"); $_ =~ s/tree\s+SIMTREE\s+=\s+//g;
	print TREE $_;
	close TREE;
    }
}
...
rmfile ("random-tree-non-MC.dat");
$cmd ="qmc/modify-edge-lengths.pl random-tree.dat";
cmd($cmd);
if (! check_exists("random-tree-non-MC.dat")) {
    die "ERROR ERROR. file random-tree-non-MC.dat was not created\n\n";
}

my $new_tree = decrease_names("random-tree-non-MC.dat");

open (OUT, ">random-tree-non-MC-new.dat");
print OUT $new_tree;
close OUT;

$model_tree = $new_tree;
my $non_weighted_model_tree =  $new_tree;
$non_weighted_model_tree =~ s/:-?[0-9]+\.[0-9]+//g;  # remove length
print "model tree after length remove is $non_weighted_model_tree\n";
$model_tree_fname = "modelTrees/model-tree-n${ntaxa}.dat";
$non_weighted_model_tree_fname =  "modelTrees/nw_model-tree-n${ntaxa}.dat";
open (OUT, ">$model_tree_fname");
print OUT $model_tree;
close OUT;


open (OUT, ">$non_weighted_model_tree_fname");
print OUT $non_weighted_model_tree;
close OUT;
...
foreach $corr (@correct)
{
	...
	{
	my  $orig_qrt_file = "r${replicateNo}_orig_qrt_n${ntaxa}_c100_qf${qnum_factor}";
	my  $qrt_file = "r${replicateNo}_qrt_n${ntaxa}_c${corr}_qf${qnum_factor}";
...
# qrt with weights
  ...
  my $command = "$max_cut_dir/tree_quartets tree_file=$tree_name out_qrt_file=$out_qrt_file pcorrect=$corr $weighted_pcorrect select=random qnum_factor=$qnum_factor ntaxa=$ntaxa weights=on";
  cmd($command);
...
sub help{
    print "USAGE:\n\n";
    print "perl data_generator.pl  -ntaxa n \n\n";   
    print "where\n";
